...
if __name__ == '__main__':
    ...
    for frame in tqdm([timedelta(minutes=24, seconds=36), timedelta(minutes=15, seconds=36),
                       timedelta(minutes=20, seconds=36), timedelta(minutes=24, seconds=38),
                       timedelta(minutes=24, seconds=0)]):
        debut = time()
        PILImages = []
        Boxes = []
        nb_events, Events_dans_frame = doc.nb_event_dans_frame(frame, returnEvents=True)
        for event in Events_dans_frame:
            docCopie = doc.doc_event_donne(event=event)
            t = ctx.make_track()
            t.populate(docCopie)
            ...
            Boxes.append(resultats_libass.to_box().to_pil(SIZE))

        from PIL import Image
        base = Image.new(mode="RGBA", size=SIZE, color=(255, 255, 255, 0))
        for image in PILImages:
            base = Image.alpha_composite(base, image)
        for border in Boxes:
            base = Image.alpha_composite(base, border)
        # print(f"Fin en {time()-debut}")

    profiler.disable()
    profiler.dump_stats(join(fileRoot, "logs", "RendererCleanLogs.prof"))
